...
class Kick(commands.Cog):
    ...
    async def _kick(self, ctx, member : discord.Member, *, reason=None):
        ...
        embed = discord.Embed(title="Kick", color=discord.Color.from_rgb(197,197,197))
    ...
    async def kick_error(self, ctx, error):
        if isinstance(error, commands.MissingPermissions):
            embed = discord.Embed(title="Kick", description="Vous n'avez pas la permission d'éxecuter cette commande.", color=discord.Color.from_rgb(197,197,197))
            embed.set_footer(text="Bot by LoliChann", icon_url=f"https://i.pinimg.com/564x/d5/d6/ff/d5d6ff7f3a344085dbffc4a9a34f538e.jpg")
            await ctx.send(embed=embed)
